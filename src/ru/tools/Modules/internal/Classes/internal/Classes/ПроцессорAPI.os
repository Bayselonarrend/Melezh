...
Функция ОсновнаяОбработка(Знач Контекст, Знач Путь) Экспорт
	...
	Если Не ОбработчикСеансов.АвторизованныйСеанс(Контекст) Тогда
		...
		Возврат Инструментарий.ОшибкаОбработки(Контекст, 401, "Ошибка авторизации. Пожалуйста, обновите страницу");
	КонецЕсли;
...
Функция ВернутьИнформациюОСобытии(Контекст)
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(ПутьЛогов) Тогда
			Возврат Инструментарий.ОшибкаОбработки(Контекст, 410, "Не указан путь логов для поиска!");
		КонецЕсли;
		
		ПутьЛога = НайтиФайлы(ПутьЛогов, КлючЛога, Истина);
		
		Если Не ЗначениеЗаполнено(ПутьЛога) Тогда
			Возврат Инструментарий.ОшибкаОбработки(Контекст, 404, "Запись не найдена. Возможно, путь к каталогу логов был изменен!");
		Иначе
		...
		Если Не ОсновнойОбъект.Существует() Тогда
			...
			Возврат Инструментарий.ОшибкаОбработки(Контекст, 500, Ошибка);
	...
	Исключение
		Результат = Инструментарий.ОшибкаОбработки(Контекст, 500, СтрШаблон("Ошибка получения информации: %1", ОписаниеОшибки()));
	КонецПопытки;
