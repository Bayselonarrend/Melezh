...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено)
	...
	Заголовок = СтрШаблон("Version %1 - %2"
		, ДанныеТекущего["version"]
		, Формат(ТекущаяДата(), "ДФ=dd.MM.yyyy"));
		
	ДанныеТекущего.Вставить("title", Заголовок);
...
Процедура СоздатьДрафтGithub(Знач Данные)
	...
	МассивИзмененийRu  = Новый Массив;
	МассивИзмененийEn  = Новый Массив;

	Для Каждого Изменение Из Данные["changes"] Цикл

		МассивИзмененийRu.Добавить(СтрШаблон("+ %1", Изменение["description_ru"]));
		МассивИзмененийEn.Добавить(СтрШаблон("+ %1", Изменение["description_en"]));

	КонецЦикла;

	ТекстДрафта = СтрШаблон(ШаблонДрафта
		, СтрСоединить(МассивИзмененийRu, Символы.ПС)
		, СтрСоединить(МассивИзмененийEn, Символы.ПС)
		, Версия);
	...
	РезультатОтвет = Результат.ВернутьОтвет();

	Если РезультатОтвет.КодСостояния > 299 Тогда
		ВызватьИсключение Результат.ВернутьОтветКакСтроку();
	Иначе
		CommonTools.СообщитьПроцесс("GitHub: Success");
	КонецЕсли;
...
Процедура СоздатьДрафтSourcecraft(Знач Данные)
	...
	Если РезультатОтвет.КодСостояния > 299 Тогда
		ВызватьИсключение Результат.ВернутьОтветКакСтроку();
	Иначе
		CommonTools.СообщитьПроцесс("SourceCraft: Success");
	КонецЕсли;
