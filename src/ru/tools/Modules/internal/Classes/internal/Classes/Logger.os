...
Процедура ЗаписатьЛог(Контекст, ТелоЗапроса, Знач Результат) Экспорт
	...
	Попытка
		...
		UUIDЗапроса   = СтрШаблон("%1-%2-%3", Формат(ДатаЗапроса, "ДФ=hh-mm-ss"), Идентификатор, Обработчик);
		...
		ЗаписыватьТелоЗапроса       = ?(ЗначениеЗаполнено(ЗаписыватьТелоЗапроса), ПолучитьБулево(ЗаписыватьТелоЗапроса), Ложь);

		ЗаписыватьЗаголовкиЗапроса  = ХранилищеНастроек.ВернутьНастройку("logs_req_headers");
		ЗаписыватьЗаголовкиЗапроса  = ?(ЗначениеЗаполнено(ЗаписыватьЗаголовкиЗапроса), ПолучитьБулево(ЗаписыватьЗаголовкиЗапроса), Ложь);

		ЗаписыватьТелоОтвета        = ХранилищеНастроек.ВернутьНастройку("logs_res_body");
		ЗаписыватьТелоОтвета        = ?(ЗначениеЗаполнено(ЗаписыватьТелоОтвета), ПолучитьБулево(ЗаписыватьТелоОтвета), Ложь);
		
		МаксимальныйРазмерЗапроса   = ХранилищеНастроек.ВернутьНастройку("logs_req_max_size");
		МаксимальныйРазмерЗапроса   = ?(ЗначениеЗаполнено(МаксимальныйРазмерЗапроса), Число(МаксимальныйРазмерЗапроса), 0);

		МаксимальныйРазмерОтвета    = ХранилищеНастроек.ВернутьНастройку("logs_res_max_size");
		МаксимальныйРазмерОтвета    = ?(ЗначениеЗаполнено(МаксимальныйРазмерОтвета), Число(МаксимальныйРазмерОтвета), 0);
		...
		Если ЗаписыватьТелоЗапроса Тогда
			...
			МаксимальныйРазмерЗапроса = МаксимальныйРазмерЗапроса;
...
Функция ВернутьДействия(Обработчик, Дата) Экспорт
	...
	ПутьЛогов = СтрШаблон("%1/%2/%3", ПутьЛогов, Обработчик, Дата);
...
Функция ПолучитьБулево(Знач Значение)
