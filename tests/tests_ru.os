...
&Порядок(17)
&Тест
Процедура ОбъявитьПеременную() Экспорт

	Опции = Новый Структура;
	Опции.Вставить("proj", ПутьПроекта);
	Опции.Вставить("var" , "var1");

	_Результат = TestTools.ВыполнитьТестCLI("ОбъявитьПеременную", Опции);
	
	Ожидаем.Что(_Результат["result"]).Равно(Истина);
	
	TestTools.ЗаписатьЛог(_Результат, "ОбъявитьПеременную");
