...
Функция ПолучитьРезультатОбработки(Знач Параметры)
	...
	ИначеЕсли Метод = "hashsum" Тогда
		...
		Возврат BuildHash.GetHashSum();

	ИначеЕсли Метод = "libsum" Тогда

		Возврат ПолучитьХешСуммуСборкиOint();
...
Функция ПолучитьХешСуммуСборкиOint()

	Попытка 
		ИнструментыOPI = ЗагрузитьСценарий("../../../oint/tools/Modules/internal/Modules/OPI_Инструменты.os");
	Исключение
		ИнструментыOPI = Неопределено;
	КонецПопытки;

	Если ИнструментыOPI <> Неопределено Тогда
		Возврат ИнструментыOPI.ПолучитьХешСуммуПоследнейСборки();
	Иначе
		Возврат "";
	КонецЕсли;
