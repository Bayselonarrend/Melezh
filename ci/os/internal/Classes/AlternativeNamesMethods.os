...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено)
	...
	ДобавитьДублеры(Новый Структура, Языки);

КонецПроцедуры

Процедура ДобавитьДублеры(СоответствиеНесовпадающихИмен, Языки) Экспорт
		
	ФайлыOint = НайтиФайлы("./src/ru", "*.os", Истина);
...
Процедура СоздатьДублеры(ФайлыДублеров)
	...
	Для Каждого ФайлДублера Из ФайлыДублеров Цикл
		...
		ТекстМодуля     = СтрЗаменить(ТекстМодуля, "oint-cli", "ointcli");
	...
	Для Каждого Оригиналы Из СоответствиеМетодовОригиналов Цикл
		...
		НачалоДополнений = СтрНайти(ТекстОригинала, "#Region Alternate");

		Если НачалоДополнений <> 0 Тогда
			ТекстОригинала = Лев(ТекстОригинала, НачалоДополнений - 1);
			ТекстОригинала = СокрП(ТекстОригинала);
		КонецЕсли;
