...
Процедура ОсновнаяОбработка()
	...
	Обработчик = Новый CLIMethods();
	Обработчик = Новый DictionariesMethods();
	Обработчик = Новый LocalizationMethods();
	Обработчик = Новый AlternativeNamesMethods();
	
	МетодыДобавленияДублеров.ДобавитьДублеры(Новый Структура, Языки);
