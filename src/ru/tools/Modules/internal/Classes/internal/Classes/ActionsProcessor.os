...
Процедура Инициализировать(ОбъектОПИ_, МодульПрокси_, МенеджерСоединений_, Логгер_, ХранилищеНастроек_) Экспорт
    ...
    ПользовательскийКэш      = Новый Соответствие();
...
Функция ВыполнитьУниверсальнуюОбработку(Контекст, Обработчик, Параметры, ТелоЗапроса, Путь) Экспорт
    ...
    Иначе
        ...
        Если Не ТипЗнч(Ответ) = Тип("ДвоичныеДанные") 
            ...
            И Не ТипЗнч(Контекст) = Тип("Структура") Тогда
...
Функция ПолучитьЗначениеИзКэша(Знач Имя) Экспорт
    Возврат ПользовательскийКэш.Получить(Имя);
КонецФункции

Процедура ЗаписатьЗначениеВКэш(Знач Имя, Знач Значение) Экспорт
    ПользовательскийКэш.Вставить(Имя, Значение);
КонецПроцедуры

Процедура УдалитьЗначениеИзКэша(Знач Имя) Экспорт
    ПользовательскийКэш.Удалить(Имя);
КонецПроцедуры
