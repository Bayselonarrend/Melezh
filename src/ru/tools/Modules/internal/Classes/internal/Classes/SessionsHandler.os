...
Функция АвторизоватьСеанс(Контекст) Экспорт
	...
	IPКлиента = Контекст.Запрос.Заголовки.Получить("X-Forwarded-For");
	IPКлиента = ?(ЗначениеЗаполнено(IPКлиента), IPКлиента, Контекст.Соединение.УдаленныйIpАдрес);

	Если ПроверитьБлокировкуАвторизации(IPКлиента) Тогда

		Результат = Toolbox.ОшибкаОбработки(Контекст, 429, "Превышено число неудачных попыток авторизации. Повторите попытку позже");
		Toolbox.ВывестиОшибкуВЛог(СтрШаблон("Заблокированный IP:", IPКлиента));
