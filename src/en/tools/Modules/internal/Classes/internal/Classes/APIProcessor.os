...
Function ReturnModuleText(Context)

	Try

		ModuleName = String(Context.Request.Parameters["file"]);;
		Result = ExtensionsProcessor.GetExtensionText(ModuleName);
...
Function SaveModuleText(Context)

	Try

		SavingStructure = Toolbox.GetJSON(Context);
		ModuleName = SavingStructure["file"];
