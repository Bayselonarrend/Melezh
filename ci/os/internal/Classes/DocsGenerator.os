...
Процедура СоздатьДокументацию()
	...
	Словарь = CommonTools.ПрочитатьФайлJSON("./service/dictionaries/en.json");

	Для Каждого Язык Из Языки Цикл

		ТекущийЯзык         = Язык;
		Расположение        = "./src/" + Язык;
		СПодкаталогами      = Истина;
		КаталогДокументации = "./docs/" + Язык + "/md/Melezh";
...
Процедура СоздатьФайлыДокументации()
	...
	ТекущийМодуль.Прочитать(СтрШаблон("%1/tools/Modules/IntegrationProxy.os", Расположение), "UTF-8");
	РазобратьМодуль(ТекущийМодуль, "IntegrationProxy");
...
Процедура СоздатьСтраницуДокументации(СтруктураМетода, Счетчик = 1)
	...
	ОписаниеСтраницы = СтрШаблон(ШаблонОписания, СтруктураМетода["Заголовок"], "Melezh");
...
Процедура РазобратьМодуль(ТекущийМодуль, ИмяМодуля)
	...
	Попытка
		СтруктураМодуля = Парсер.Разобрать(СтрЗаменить(ТекстМодуля, "oint-cli", "ointcli"));
	Исключение
		А = ОписаниеОшибки();
	КонецПопытки;
...
Процедура ПолучитьДанныеМодуля(СтруктураОбщихДанных, ТекущийМодуль)
	...
	СтруктураОбщихДанных.Вставить("Каталог"   , "Console-Interface");
...
Процедура ОчиститьКаталогиДокументов()
	...
	Постоянные.Добавить("Web-Interface");
...
Функция ПолучитьВызовCLI(СтруктураМетода)
	...
	ПутьCLI  = КаталогCLI + СтруктураМетода["ИмяМетода"];
...
Функция ПолучитьРезультатМетода(Знач Модуль, Знач Метод)
	...
	КаталогРаздела      = Новый Файл(КаталогЛогов);

	Если Не КаталогРаздела.Существует() Тогда
		Возврат Результат;
	КонецЕсли;

	ПутьЛога = КаталогЛогов + "/" + Метод + ".log";
