...
Функция ПодключитьФайлРасширения(ФайлРасширения, Тест = Ложь) Экспорт
    ...
    Попытка
        ...
        ДобавитьРасширениеВКэш(ФайлРасширения.ИмяБезРасширения, ФайлРасширения.ПолноеИмя, ТаблицаПараметров["methods"].Количество());
...
Функция СформироватьСтруктуруОписанияПараметра(Знач Метод, Знач ПараметрМетода)
    ...
    Если КоличествоЭлементов < 4 Тогда
        ВызватьИсключение("Недостаточный набор данных в док. комментарии: " + Метод.Signature.Name);
    КонецЕсли;
    ...
    Если КоличествоЭлементов > 5 Или СтрНайти(Имя, " ") > 0 Тогда
        ВызватьИсключение("Некорректный документирующий комментарий в методе: " + Метод.Signature.Name);
    КонецЕсли;
