...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено)
	...
	КаталогОригинала = CommonTools.ПолучитьКаталогОсновныхИсходников();
	ВсеФайлы         = НайтиФайлы(КаталогОригинала, "*", Истина);
    
	СоздатьНаборСловарей(ВсеФайлы);

	Если ЕстьНовыеСлова Тогда
		Сообщить("Dictionaries contain new words without translation. Translation and CLI creation not completed!");
		ЗавершитьРаботу(111);
	КонецЕсли;

КонецПроцедуры

Процедура СоздатьНаборСловарей(Знач ФайлыМодулей)
		
	ПутьСловаря = "./service/dictionaries/en.json";
	ПутьСловаряOint = "../OpenIntegrations/service/dictionaries/en.json";
	
	ФайлСловаря     = Новый Файл(ПутьСловаря);
		
	Если ФайлСловаря.Существует() Тогда
		СоздатьСловарь(ФайлыМодулей, ПутьСловаря, ПутьСловаряOint);
	КонецЕсли;
	
КонецПроцедуры

Процедура СоздатьСловарь(Знач ФайлыМодулей, Знач ПутьСловаря, Знач ПутьСловаряOint = "") Экспорт
	
	МассивСимволов = CommonTools.ПолучитьМассивСимволов();
	
	СоответствиеСловаря = CommonTools.ПрочитатьФайлJSON(ПутьСловаря);
	СписокСлов          = Новый СписокЗначений();
	
	Для Каждого Модуль Из ФайлыМодулей Цикл
		
		Если Модуль.ЭтоКаталог() 
			Или Модуль.Расширение = ".html"
			Или Модуль.Расширение = ".js"
			Или Модуль.Расширение = ".css"
			Или Модуль.Расширение = ".png"
			Или Модуль.Расширение = ".ico"
			Или Модуль.Расширение = ".zip" 
			Или СтрНайти(Модуль.ПолноеИмя, "EmbeddedLanguageParser") <> 0 Тогда
			Продолжить;
		КонецЕсли;
	...
	ФайлСловаряOint = Новый Файл(ПутьСловаряOint);

	Если ФайлСловаряOint.Существует() Тогда
		СкорректироватьСловарьНаОсновеOint(СоответствиеСловаря, ПутьСловаряOint);
	КонецЕсли;
...
Функция НайтиКириллицу(Знач Слово)
	...
	Алфавит   = CommonTools.ПолучитьАлфавит();
...
Процедура СкорректироватьСловарьНаОсновеOint(СоответствиеСловаря, ПутьСловаряOint)
	
	СоответствиеСловаряOint = CommonTools.ПрочитатьФайлJSON(ПутьСловаряOint);
	
	Для Каждого ЧастьСловаря Из СоответствиеСловаря Цикл
		Если СоответствиеСловаряOint.Получить(ЧастьСловаря.Ключ) = Неопределено Тогда
			СоответствиеСловаряOint.Вставить(ЧастьСловаря.Ключ, ЧастьСловаря.Значение);
		КонецЕсли;
	КонецЦикла;
	
	СоответствиеСловаря = СоответствиеСловаряOint;
