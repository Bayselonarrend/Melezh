/**
 * Расширенный cron Chronos / rust `cron`: 7 полей
 * «сек мин час день_мес месяц день_недели год»
 * Дни недели в движке: числа 1–7, где 1 = вс, 2 = пн … 7 = сб (см. days_of_week в crate cron).
 * Подписи дней недели в UI задаются в scheduler-view (Пн…Вс → те же ординалы).
 */
...
/**
 * Простые поля: *, одиночное число, списки и диапазоны из цифр.
 * Без шагов /, без имён месяцев, без L # ?
 */
...
export function tryParseScheduleString(str) {
    ...
    if (parts.length !== 7) return { ok: false, reason: 'Нужно ровно 7 полей через пробел.' };
    ...
    if (yearT === '*') {
        ...
        if (!y) {
            return {
                ok: false,
                reason: 'Расписание установлено вручную',
            };
        }
