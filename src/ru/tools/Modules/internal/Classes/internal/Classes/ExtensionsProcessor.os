    ...
    Для Каждого ТекущийКаталогРасширений Из КаталогиРасширений Цикл
        ...
        Для Каждого ФайлРасишрения Из ФайлыРасширений Цикл
            ...
            Попытка
                ...
                ПроцессорДействий.ПодключитьСценарийРасширения(ФайлРасишрения.ПолноеИмя, ФайлРасишрения.ИмяБезРасширения);

            Исключение

                ОписаниеПроблемы = СтрШаблон("Ошибка применения расширения %1", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));

                ДобавитьРасширениеВКэш(ФайлРасишрения.ИмяБезРасширения, ФайлРасишрения.ПолноеИмя, ОписаниеПроблемы);
...
Функция СоздатьФайлРасширения(ИмяМодуля, КаталогСоздания) Экспорт
    ...
    Если Не ПереданДоступный Тогда
        Возврат Новый Структура("result,error", Ложь, "Передан некорректный каталог расширений");
    КонецЕсли;

    Если КэшРасширений.Получить(ИмяМодуля) <> Неопределено Тогда
        Возврат Новый Структура("result,error", Ложь, "Расширение с таким именем уже существует");
    КонецЕсли;
