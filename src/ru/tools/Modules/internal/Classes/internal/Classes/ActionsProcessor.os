...
Процедура Инициализировать(ОбъектОПИ_, МодульПрокси_, МенеджерСоединений_, Логгер_, ХранилищеНастроек_) Экспорт
    ...
    СписокАктивныхРасширений = Новый СписокЗначений;
...
Процедура ПодключитьСценарийРасширения(Знач Путь, Знач Имя) Экспорт
    ...
    Попытка
        ...
        СписокАктивныхРасширений.Добавить(Имя);
    Исключение
        Сообщить("Не удалось подключить сценарий расширения. Возможно, он уже подключен");
    КонецПопытки;
...
Функция ВыполнитьУниверсальнуюОбработку(Контекст, Обработчик, Параметры)
    ...
    Иначе
        ...
        Если СписокАктивныхРасширений.НайтиПоЗначению(Команда) <> Неопределено Тогда
            ТекстВыполнения = СтрШаблон("%1 = Новый %1;", Команда) 
                + Символы.ПС 
                + ТекстВыполнения;
        КонецЕсли;
