...
Процедура ПриСозданииОбъекта(Знач ДанныеПроекта_ = Неопределено)
	...
	Файлы            = НайтиФайлы(КаталогОригинала, "*", Истина);

	ПеревестиПроект();

КонецПроцедуры

Процедура ПеревестиПроект()
...
Процедура ПеревестиПоСловарю();
	...
	Для Каждого ФайлМодуля Из ФайлыМодулей Цикл
		...
		Если ФайлМодуля.ЭтоКаталог() 
			...
			Или СтрНайти(ФайлМодуля.Имя, "code-editor-view") <> 0
			Или СтрНайти(ФайлМодуля.Имя, "prism") <> 0
			Или СтрНайти(ФайлМодуля.Имя, "EmbeddedLanguageParser") <> 0
			...
			Или ФайлМодуля.Расширение = ".css" Тогда
			Продолжить;
		КонецЕсли;
