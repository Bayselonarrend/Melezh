...
Функция ВыполнитьТестCLI(Знач Метод, Знач Опции, Знач Записывать = Истина) Экспорт
    ...
    ЭтоWindows = OPI_Инструменты.ЭтоWindows();
    ...
    ФайлДополнительный = ПолучитьИмяВременногоФайла();
    ...
    СтрокаЗапуска = СтрокаЗапуска + " --out """ + ФайлРезультата + """ --debug > """ + ФайлДополнительный + """";

    Если ЭтоWindows Тогда

        Bat = ПолучитьИмяВременногоФайла(".bat");
        ПолучитьДвоичныеДанныеИзСтроки(СтрокаЗапуска, Консоль.КодировкаВходногоПотока).Записать(Bat);
        СтрокаЗапуска = Bat;
    ...
    Если ЭтоWindows Тогда
        ...
        Попытка
            УдалитьФайлы(Bat);
        Исключение
            Сообщить("Не удалось удалить Bat файл после теста!");
        КонецПопытки;

    КонецЕсли;

    Результат = ПрочестьРезультатCLI(ФайлРезультата, ФайлДополнительный);
...
Функция ПрочестьРезультатCLI(Знач ФайлРезультата, Знач ФайлДополнительный)
    ...
    Исключение
        ...
        Попытка
            Результат =  ПолучитьСтрокуИзДвоичныхДанных(Новый ДвоичныеДанные(ФайлРезультата));
        Исключение
            Попытка
                Результат = ПолучитьСтрокуИзДвоичныхДанных(Новый ДвоичныеДанные(ФайлДополнительный));;
            Исключение
                Результат = null;
            КонецПопытки;
        КонецПопытки;
