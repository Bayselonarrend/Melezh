...
Функция ЗапуститьПриложениеВКонсоли(Знач СтрокаВызова, Знач Каталог = Неопределено, Знач ТольОшибки = Ложь) Экспорт
	
	ИВФ          = ПолучитьИмяВременногоФайла();
	СтрокаВызова = СтрокаВызова + СтрШаблон("> %1 2>&1", ИВФ);
	
	КодВозврата = 0;
	ЗапуститьПриложение(СтрокаВызова, Каталог, Истина, КодВозврата);
	
	ФайлРезультата = Новый Файл(ИВФ);
	Если ФайлРезультата.Существует() Тогда
		
		Если Не ТольОшибки Или КодВозврата <> 0 Тогда
			Сообщить(ПолучитьСтрокуИзДвоичныхДанных(Новый ДвоичныеДанные(ИВФ)));
			УдалитьФайлы(ИВФ);
		КонецЕсли;
...
Функция ЗапуститьПриложениеВКонсолиБезВывода(Знач СтрокаВызова, Знач Каталог = Неопределено) Экспорт
