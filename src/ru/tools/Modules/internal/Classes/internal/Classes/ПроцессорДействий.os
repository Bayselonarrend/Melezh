...
Процедура Инициализировать(ОбъектОПИ_, МодульПрокси_, МенеджерСоединений_, Логгер_) Экспорт
...
Функция ОсновнаяОбработка(Знач Контекст, Знач Путь) Экспорт
    ...
    Попытка
        ...
        Иначе
            Результат = Инструментарий.ОшибкаОбработки(Контекст, 404, "Обработчик не найден!");
        КонецЕсли;
    ...
    Логгер.ЗаписатьЛог(Контекст, ТелоЗапроса);
...
Функция ВыполнитьОбработку(Контекст, Обработчик, ТелоЗапроса)
    ...
    Если Обработчик["active"] = 0 Тогда
        Возврат Инструментарий.ОшибкаОбработки(Контекст, 403, "Данный обработчик был отключен на стороне сервера!");
    КонецЕсли;
    ...
    Если Не Метод = МетодПроверки Тогда
        Возврат Инструментарий.ОшибкаОбработки(Контекст, 405, "Метод " + Метод + " недоступен для этого обработчика!");
    КонецЕсли;
    ...
    ИначеЕсли МетодОбработчика = "FORM" Тогда
        ...
        Результат = ВыполнитьОбработкуFormData(Контекст, Обработчик, ТелоЗапроса);

    Иначе

        Результат = Инструментарий.ОшибкаОбработки(Контекст, 405, "Метод " + Метод + " недоступен для этого обработчика!");
...
Функция ВыполнитьОбработкуFormData(Контекст, Обработчик, ТелоЗапроса)
