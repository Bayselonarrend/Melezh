...
Функция ВернутьСписокФункций(Контекст)
	...
	Попытка
		...
		Для Каждого ТекущаяФункция Из Индекс["methods"] Цикл

			ФункцияОПИ = ТекущаяФункция["name"];
...
Функция ВернутьСписокАргументов(Контекст)
	...
	Попытка
		...
		Индекс           = ОбъектОПИ.ПолучитьИнформациюОМетоде(Библиотека, Метод);
		СписокАргументов = Индекс["params"];
		
		МассивОпций   = Новый Массив;
		
		Для Каждого Аргумент Из СписокАргументов Цикл
			
			ТекущийПараметр = СтрЗаменить(Аргумент["name"], "--", "");

			Если ТекущийПараметр = "melezhcontext" Тогда
				Продолжить;
			КонецЕсли;

			ЗначениеПоУмолчанию = Аргумент["default"];
			ОписаниеПараметра   = Аргумент["description"];
			ОписаниеПараметра   = ?(ЗначениеПоУмолчанию = Неопределено
				, ОписаниеПараметра
				, ОписаниеПараметра + СтрШаблон(" (необяз. по ум. - %1)", ЗначениеПоУмолчанию));

			МассивОпций.Добавить(Новый Структура("arg,description", ТекущийПараметр, ОписаниеПараметра));
...
Процедура ИзменитьРегистрНазваний(Синоним)
	...
	СоответствиеНазваний.Вставить("Click house", "ClickHouse");
	СоответствиеНазваний.Вставить("Green API", "GreenAPI (WhatsApp)");
	СоответствиеНазваний.Вставить("Green max", "GreenAPI (Max)");
