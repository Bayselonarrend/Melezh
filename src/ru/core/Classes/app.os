...
Процедура ОсновнойОбработчик()
	...
	Версия         = СоставАддона.ПолучитьВерсию();
	ТаблицаМетодов = СоставАддона.ПолучитьСостав();
...
Функция ПолучитьРезультатОбработки(Знач Параметры)
	...
	Если Не ЗначениеЗаполнено(Метод) Тогда
		...
		Если ЭтоСправка Тогда
			Справка.ВывестиСправкуПоМетодам(ТаблицаМетодов);
		Иначе
			Справка.ВывестиНачальнуюСтраницу(Версия);
		КонецЕсли;
