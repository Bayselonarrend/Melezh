...
Процедура ПеревестиПроект(Знач ФайлыМодулей, Знач КаталогФайлов) Экспорт
	...
	ИменаМодулей.Вставить("ОбработчикЗапросов", Истина);
	ИменаМодулей.Вставить("ПроксиИнтеграций", Истина);
	ИменаМодулей.Вставить("Справка", Истина);
	ИменаМодулей.Вставить("СоставАддона", Истина);
...
Процедура ПолучитьТаблицуСловаря(ПутьКСловарю)
	...
	Для Каждого Элемент Из ДанныеСловаря Цикл
		...
		НоваяСтрокаСловаря.ИмяМодуля = ИменаМодулей.Получить(Элемент.Ключ) <> Неопределено;
